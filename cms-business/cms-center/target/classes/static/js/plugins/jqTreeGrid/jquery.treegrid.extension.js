(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        target.renderRow = function(tr,item){
            $.each(options.columns, function(index, column) {
                ...
                if(index==0&&column.field=='selectItem'){
                    hasSelectItem = true;
                    var td = $('<td style="text-align:portal;width:36px"></td>');
                    if(column.radio){
        ...
        target.load = function(parms){
            ...
            target.append(tbody);
            // 添加加载loading
            var _loading = '<tr><td colspan="'+options.columns.length+'"><div style="display: block;text-align: portal;">正在努力地加载数据中，请稍候……</div></td></tr>'
            tbody.html(_loading);
            // 默认高度
            if(options.height){
            ...
            $.ajax({
                ...
                success : function(data, textStatus, jqXHR) {
                    ...
                    if(!data||data.length<=0){
                        var _empty = '<tr><td colspan="'+options.columns.length+'"><div style="display: block;text-align: portal;">没有记录</div></td></tr>'
                        tbody.html(_empty);
                        return;
                    }
                ...
                error:function(xhr,textStatus){
                    var _errorMsg = '<tr><td colspan="'+options.columns.length+'"><div style="display: block;text-align: portal;">'+xhr.responseText+'</div></td></tr>'
